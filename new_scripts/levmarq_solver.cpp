...
int main(int argc, char* argv[])
{
    try {
        ...
        for (int i = 0; i < npar; ++i) {
            if (names[i] == "r2") {
                limits[i].first  = std::max(limits[i].first,  1e-5);
                limits[i].second = std::min(limits[i].second, 2.0);
            }
            if (names[i] == "r1") {
                limits[i].first  = std::max(limits[i].first,  1e-5);
                limits[i].second = std::min(limits[i].second, 2.0);
                                ...
                                vector<double>& fit_out) -> bool
        {
            ...
            try {
                ...
                if (verbose) std::cerr << DIM << "  [eval failed: unknown]"
                                    << RESET << std::endl;
                return false;
            }
            ++fev_count;
            ...
            for (int i = 0; i < ndata; ++i) {
                if (!std::isfinite(fitv[i])) return false;
                double sigma = data[i].ferr;
                if (!(sigma > 0.0)) sigma = 1e-10;
                double w = (data[i].weight > 0.0) ? data[i].weight : 1.0;
                resid[i] = std::sqrt(w) * (data[i].flux - fitv[i]) / sigma;
                            ...
                            bool central = false) -> bool
        {
            ...
            for (int j = 0; j < npar; ++j) {
                ...
                bool ok = compute_residuals(p_pert, resid_pert, chisq_dummy, fit_dummy);

                // If perturbation hits a Roche::face error, halve the step
                // a few times before giving up.
                int retry = 0;
                while (!ok && retry < 4) {
                    hj *= 0.5;
                    p_pert[j] = pars[j] + sign * hj;
                    actual_h  = sign * hj;
                    ok = compute_residuals(p_pert, resid_pert, chisq_dummy, fit_dummy);
                    ++retry;
                }
                if (!ok) {
                    if (sign > 0) {
                ...
                for (int i = 0; i < nresid; ++i) {
                    double dr = (resid_pert[i] - resid0[i]) * inv_h;
                    if (!std::isfinite(dr)) dr = 0.0;
                    J[i][j] = dr;
                }
            }
        ...
        std::cerr << BRIGHT_RED << "Fatal: " << e.what()
                  << RESET << std::endl;
        return 2;
    } catch (...) {
        std::cerr << BRIGHT_RED << "Fatal: unknown exception"
                  << RESET << std::endl;
        return 2;
    }
}

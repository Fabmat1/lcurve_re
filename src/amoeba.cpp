...
namespace Subs {
    ...
    void
    amoeba_get_psum(const std::vector<std::pair<Subs::Array1D<double>, double> > &params, Subs::Array1D<double> &psum);
}
...
void Subs::amoeba(std::vector<std::pair<Subs::Array1D<double>, double> > &params, double ftol, int nmax, Afunc &func, int &nfunc) {
    ...
    for (;;) {
        ...
        for (
                ...
                i++) {
            if (params[i].second <= params[ilo].second)
                ilo = i;
            if (params[i].second > params[ihi].second) {
                inhi = ihi;
                ihi = i;
            } else if (params[i].second > params[inhi].second &&
                       int(i)
                       != ihi) {
                inhi = i;
            }
        }
        double rtol = 2. * fabs(params[ihi].second - params[ilo].second) /
                      (fabs(params[ihi].second) + fabs(params[ilo].second) + TINY);

        if (rtol < ftol) {
            std::swap(params[0]
                              .second, params[ilo].second);
            for (
                    int i = 0;
                    i < ndim;
                    i++)
                std::swap(params[0]
                                  .first[i], params[ilo].first[i]);
            break;
        }

        if (nfunc >= nmax) {
            std::cerr << "amoeba: nmax = " << nmax << " exceeded." <<
                      std::endl;
            break;
        }

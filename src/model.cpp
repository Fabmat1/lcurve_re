...
Model::Model(const json config) {
    ...
    if (!config.contains("model_parameters") || !config["model_parameters"].is_object()) {
        throw runtime_error("Missing or invalid 'model_parameters'");
    }

    for (auto &[p, v] : config["model_parameters"].items()) {
        if (v.is_string()) {
            pv[p] = v.get<string>();
            names[p] = true;
        }
        else {
            throw runtime_error("Malformed model parameter value for parameter '" + p + "'");
        }
    }
    ...
    bool ok = true;
    for(auto &[name, defined] : names){
        if(!defined){
            if(name == "pdot" || name == "third" ||
	       name == "temp_edge" || name == "absorb_edge"){
                cerr << name
                          << " was not defined; will be set = 0" << endl;
                pv[name] = "0 1.e-10 1.e-10 0 0";
            }else if(name.substr(0,4) != "stsp" && name.substr(0,4) != "uesp"){
                cerr << name << " was not defined " << endl;
                ok = false;
            }
        }
    }
    if(!ok) throw runtime_error("One or more parameters were not initialised");
    ...
    if(names["stsp11_long"] || names["stsp11_lat"] ||
       names["stsp11_fwhm"] || names["stsp11_tcen"]){
        ...
        if(!(names["stsp11_long"] && names["stsp11_lat"] &&
             names["stsp11_fwhm"] && names["stsp11_tcen"]))
            throw runtime_error("One or more of the star spot 11 parameters were not initialised");
    ...
    if(names["stsp12_long"] || names["stsp12_lat"] ||
       names["stsp12_fwhm"] || names["stsp12_tcen"]){
        ...
        if(!(names["stsp12_long"] && names["stsp12_lat"] &&
             names["stsp12_fwhm"] && names["stsp12_tcen"]))
            throw runtime_error("One or more of the star spot 12 parameters were not initialised");
    ...
    if(names["stsp13_long"] || names["stsp13_lat"] ||
       names["stsp13_fwhm"] || names["stsp13_tcen"]){
        ...
        if(!(names["stsp13_long"] && names["stsp13_lat"] &&
             names["stsp13_fwhm"] && names["stsp13_tcen"]))
            throw runtime_error("One or more of the star spot 13 parameters were not initialised");
    ...
    if(names["stsp21_long"] || names["stsp21_lat"] ||
       names["stsp21_fwhm"] || names["stsp21_tcen"]){
        if(!(names["stsp21_long"] && names["stsp21_lat"] &&
             names["stsp21_fwhm"] && names["stsp21_tcen"]))
            throw runtime_error("One or more of the star spot 21 parameters were not initialised");
    ...
    if(names["stsp22_long"] || names["stsp22_lat"] ||
       names["stsp22_fwhm"] || names["stsp22_tcen"]){
        ...
        if(!(names["stsp22_long"] && names["stsp22_lat"] &&
             names["stsp22_fwhm"] && names["stsp22_tcen"]))
            throw runtime_error("One or more of the star spot 22 parameters were not initialised");
    ...
    if(names["uesp_long1"] || names["uesp_long2"] ||
      ...
      if(!(names["uesp_long1"] && names["uesp_long2"] &&
	   names["uesp_lathw"] && names["uesp_taper"] && names["uesp_temp"]))
	throw runtime_error("One or more of uniform equatorial spot parameters were not initialised");
    ...
    delta_phase = stod(pv["delta_phase"]);
    nlat1f = stoi(pv["nlat1f"]);
    nlat2f = stoi(pv["nlat2f"]);
    nlat1c = stoi(pv["nlat1c"]);
    nlat2c = stoi(pv["nlat2c"]);
    npole = Subs::string_to_bool(pv["npole"]);

    nlatfill = stoi(pv["nlatfill"]);
    nlngfill = stoi(pv["nlngfill"]);
    lfudge = stod(pv["lfudge"]);
    llo = stod(pv["llo"]);
    lhi = stod(pv["lhi"]);
    phase1 = stod(pv["phase1"]);
    phase2 = stod(pv["phase2"]);
    wavelength = stod(pv["wavelength"]);
    ...
    if(glens1 && roche1)
      throw runtime_error("For reasons of simplicity, glens1 = 1 and roche1 = 1 are not simultaneously allowed");
    use_radii        = Subs::string_to_bool(pv["use_radii"]);
    tperiod          = stod(pv["tperiod"]);
    gdark_bolom1     = Subs::string_to_bool(pv["gdark_bolom1"]);
    gdark_bolom2     = Subs::string_to_bool(pv["gdark_bolom2"]);
    mucrit1          = stod(pv["mucrit1"]);
    mucrit2          = stod(pv["mucrit2"]);
    ...
    if(pv["limb1"] == "Poly"){
        ...
        throw runtime_error("Could not recognize the value of limb1 = " +
    ...
    if(pv["limb2"] == "Poly"){
        ...
        throw runtime_error("Could not recognize the value of limb2 = " +
...
void Model::set_param(const Subs::Array1D<double> &vpar) {
    int n = 0;
...
string Model::get_name(int i) const {
...
bool Model::is_not_legal(const Subs::Array1D<double> &vpar) const {
    int n = 0;
...
Subs::Array1D<double> Model::get_param() const {
...
Subs::Array1D<double> Model::get_range() const {
...
Subs::Array1D<double> Model::get_dstep() const {
 ...
 */
void Model::wrasc(const string& file) const {

...
namespace Lcurve {
    ...
    string Lcurve::Model::get_name(int i) const {
    ...
    vector<pair<double, double>> Lcurve::Model::get_limit() const {
        ...
        if(q.vary) temp.emplace_back(0.01, 100);
    ...
    ostream& operator<<(ostream& s, const Pparam& p) {
        s << setw(17) << setprecision(10) << fixed << p.value << " "
          << setw(8) << setprecision(4) << fixed << p.range << " "
          << setw(8) << setprecision(4) << fixed << p.dstep << " "
          << p.vary << " " << p.defined;
        return s;
    }

    /** Outputs the current values of the model parameters */
    ostream& operator<<(ostream& s, const Lcurve::Model& model){
        s << "q              = " << model.q              << "\n";

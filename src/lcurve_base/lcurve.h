...
namespace Lcurve {
    ...
    struct Model {
        ...
        void get_r1r2(double &rr1, double &rr2) const {
            if (this->use_radii) {
                ...
                double sini = sin(Subs::deg2rad(this->iangle));
                double r2pr1 = sqrt(1. - pow(sini * cos(2. * M_PI * this->cphi4.value), 2));
                double r2mr1 = sqrt(1. - pow(sini * cos(2. * M_PI * this->cphi3.value), 2));
                rr1 = (r2pr1 - r2mr1) / 2.;
                rr2 = (r2pr1 + r2mr1) / 2.;
            }
            return;
    ...
    void add_faces(Subs::Buffer1D<Lcurve::Point> &star, int &nface, double tlo, double thi, double dtheta, int nlatfill,
                   int nlngfill,
